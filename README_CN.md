...
如果用户把这个仓库链接发给你，按以下步骤操作：

1. 阅读本 README，了解所有可用 skills
2. 向用户展示每个 skill 的名称和一句话描述
3. 询问："你想安装哪些 skills？（全部 / 选择特定的）"
4. 对每个选中的 skill，检查依赖是否满足（见下方[依赖配置](#依赖配置)）
5. 如缺少 API key 或工具，引导用户完成配置
6. 将选中的 skill 文件夹复制到对应平台目录：
